...
MAX_LAG = int(os.getenv("THREECOMMASMAXLAG", "300"))  # 5 minutes
RATE_LIMIT_SECONDS = int(os.getenv("THREECOMMASRATELIMIT", "300"))  # 5 min per instrument
...
def _send_to_3commas(payload: dict) -> dict:
    ...
    if not THREECOMMAS_SECRET:
        return {"success": False, "error": "THREECOMMASSECRET not set in environment"}
    try:
