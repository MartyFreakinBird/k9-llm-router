...
def adjust_member_prob(member: str, base_prob: float, fiscal_sensitivity: float,
    ...
    # Chair protection: FLOOR the adjusted probability at 0.45 (stays in hold)
    # This prevents the Chair from being classified as "cut" when protection is active
    if chair_protection and (fiscal_score < 90 or not banking_crisis):
        adjusted_prob = max(0.45, adjusted_prob)
    ...
    # Determine if this is a "flip" — meaningful shift from hike to hold/cut
    # A member at exactly 0.50 is already a swing — moving to 0.48 is noise
    # Only count as a flip if they cross from hike territory (>=0.55) to hold/cut (<0.50)
    flip_triggered = (base_prob >= 0.55 and adjusted_prob < 0.50)
...
def grid_bot_stress_test(scenario_10y: float = 5.5,
    ...
    return {
        ...
        "recommendation": (
            ...
            f"vs hedged {_percentile(mc_hedged, 95)*100:.1f}%"
        ),
    }
