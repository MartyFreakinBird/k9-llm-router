...
def compute_gex(asset: str, top_count: int = 10) -> dict:
    ...
    # 2. Find nearest expiry with significant OI
    # Skip expiries within 4 hours (about to expire, OI has rolled)
    # Also fetch book summary here to count OI per expiry
    ...
    expiry_oi: dict = defaultdict(float)
    ...
    # Skip expiries within 4 hours, require > 1000 OI
    min_expiry_ms = now_ms + (4 * 3600 * 1000)  # 4 hours from now
    valid_expiries = sorted([
        ts for ts, oi in expiry_oi.items()
        if ts >= min_expiry_ms and oi > 1000
    ])

    if not valid_expiries:
        # Fallback: any expiry > 4 hours out
        valid_expiries = sorted([
            ts for ts in expiry_oi.keys() if ts >= min_expiry_ms
        ])

    if not valid_expiries:
        return {"error": "No valid future expiry found"}

    # Use nearest valid expiry (first one with significant OI)
    nearest_expiry_ms = valid_expiries[0]
